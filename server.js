...
// ── Keep-last-200 pruning ─────────────────────────────────────────────────────
// Called after every approve. Deletes confessions beyond the newest 200.
// Replies for pruned confessions are also cleaned up.
async function pruneConfessions() {
  try {
    const total = await confCol.countDocuments({});
    if (total <= 200) return;
    const cutoffDoc = await confCol.find({}).sort({ createdAt: -1 }).skip(200).limit(1).toArray();
    if (!cutoffDoc.length) return;
    const cutoffTs = cutoffDoc[0].createdAt;
    const deleted = await confCol.deleteMany({ createdAt: { $lte: cutoffTs } });
    if (deleted.deletedCount) {
      const remaining = await confCol.distinct('id');
      await repliesCol.deleteMany({ confessionId: { $nin: remaining } });
    }
  } catch(e) { console.warn('pruneConfessions error:', e.message); }
}

// ── Confession of the Day ─────────────────────────────────────────────────────
// Runs every hour. Picks the most-liked confession from the last 24h and pins it.
async function updateConfessionOfDay() {
  try {
    const cutoff = Date.now() - 24 * 3600 * 1000;
    const top = await confCol.find({ createdAt: { $gte: cutoff } }).sort({ likes: -1 }).limit(1).toArray();
    if (!top.length) return;
    await settingsCol.updateOne(
      { _id: 'main' },
      { $set: { cotdId: top[0].id, cotdUpdatedAt: Date.now() } }
    );
  } catch(e) { console.warn('updateConfessionOfDay error:', e.message); }
}
...
app.use('/api/thisisme', interactionLimiter);
app.use('/api/poll', interactionLimiter);
...
app.post('/api/confessions', async (req, res) => {
  const { message, clientId, category, year, branch, poll } = req.body;
  ...
  const validCategory = ['💘 Crush','🌹 Shayari','💭 Random','📊 Poll'];

  const isPoll = category === '📊 Poll' && poll && typeof poll.question === 'string';
  ...
  const confession = {
    ...
    isPoll: isPoll || false,
    pollOptions: isPoll ? {
      a: (poll.optA || 'Yes').slice(0, 60),
      b: (poll.optB || 'No').slice(0, 60),
    } : undefined,
    pollVotes:     isPoll ? { a: 0, b: 0 } : undefined,
    pollUserVotes: isPoll ? {} : undefined,
    // "This is me!" — array of clientIds who tapped it
    thisIsMe: [],
  };

  if (autoApprove) {
    await confCol.insertOne(confession);
    await pruneConfessions();
    await updateConfessionOfDay();
  ...
  // Return confessionId so frontend can track it for reply notifications
  return res.json({
...
app.post('/api/thisisme/:id', async (req, res) => {
  ...
  await confCol.updateOne({ id: req.params.id }, { $addToSet: { thisIsMe: clientId } });
  const updated = await confCol.findOne({ id: req.params.id });
  res.json({ count: updated.thisIsMe.length, list: updated.thisIsMe });
});
...
app.post('/api/poll/:id/vote', async (req, res) => {
  ...
  const updated = await confCol.findOne({ id: req.params.id });
  res.json({ pollVotes: updated.pollVotes });
});
...
connectDB().then(() => {
  ...
  // Run Confession of the Day on boot + every hour
  updateConfessionOfDay().catch(()=>{});
  setInterval(() => updateConfessionOfDay().catch(()=>{}), 60 * 60 * 1000);
